...
from quants.quant_convs_bbcu import BinaryConv2d
import torch
...
class DnCNN(nn.Module):
    def __init__(self, in_nc=1, out_nc=1, nc=64, nb=17,act_mode='BR',k=1.):
        """
        # ------------------------------------
        ...
        bias = True
        m_head = B.conv(in_nc, nc, mode='CR', bias=bias)
        m_body = [BinaryConv2d(nc,nc, kernel_size=3, bias=bias) for _ in range(nb-2)]
...
class IRCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(len(self.model)-1):
            n = self.model[i](n)
        fea.append(torch.mean(n, dim=1, keepdim=True))
        n = self.model[-1](n)
        print("xiabinsadasdasdasd")
        return x-n, fea
...
class FDnCNN(nn.Module):
    def __init__(self, in_nc=2, out_nc=1, nc=64, nb=20, act_mode='R'):
        ...
        m_head = B.conv(in_nc, nc, mode='CR', bias=bias)
        m_body = [B.conv(nc, nc, mode='CR', bias=bias) for _ in range(nb-2)]
        m_tail = B.conv(nc, out_nc, mode='C', bias=bias)
